			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					...
					$date = new \DateTime("now");
 					$timezone = $date->format("H:i:s");
					//
					$sql = "INSERT INTO attendance(employee_id, date, time_in, status) VALUES ('$id', '$date_now','$timezone', '$logstatus')";
					if($conn->query($sql)){
				...
				if($query->num_rows < 1){
					...
					if($row['time_out'] != '00:00:00'){
						...
						$sql = "UPDATE attendance SET time_out = '$timezone' WHERE id = '".$row['uid']."'";
						if($conn->query($sql)){
